...
void draw_debug_info(float dt)
{
    static const size_t debug_text_buff_sz = 1024;
    char player_pos_text[debug_text_buff_sz];
    int nprinted_bytes = 0;

    nprinted_bytes += snprintf(player_pos_text + nprinted_bytes, debug_text_buff_sz - nprinted_bytes,
             "player pos: (%0.3f,%0.3f,%0.3f)\n", s_player.pos[0], s_player.pos[1], s_player.pos[2]);

    nprinted_bytes += snprintf(player_pos_text + nprinted_bytes, debug_text_buff_sz - nprinted_bytes,
             "player vel: (%0.3f,%0.3f,%0.3f)\n", s_player.vel[0], s_player.vel[1], s_player.vel[2]);

    nprinted_bytes += snprintf(player_pos_text + nprinted_bytes, debug_text_buff_sz - nprinted_bytes,
             "camera: yaw=%0.3f, pitch=%0.3f\n", s_player.camera.yaw, s_player.camera.pitch);

    nprinted_bytes += snprintf(player_pos_text + nprinted_bytes, debug_text_buff_sz - nprinted_bytes,
             "fps: %zu\n", (size_t) floorf(1000.0f/dt));

    draw_text(&s_textrenderer, player_pos_text, strlen(player_pos_text),
              (vec2s){ .x=50, .y=s_winsize.y-50 }, TEXT_ALIGN_LEFT, 
              18, 0, s_color_white);
